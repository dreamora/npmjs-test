...
const slackMentionMapping = () => {
  ...
  if (slackMentionMappingData == null) {
    try {
      this.slackMentionMapping = JSON.parse(
        fs.readFileSync(slackMentionMappingFilePath, "utf8")
      );
    } catch (error) {
      core.warning(
        'Could not get slack mention mapping! Looking for file at "' +
          path.resolve(slackMentionMappingFilePath) +
          '".\n' +
          error
      );
      return null;
...
const listOfVariables = [
  ...
  customVariable(
    "CUSTOM_AUTHOR_PICTURE",
    () => `http://github.com/${authorName}.png?size=32`
  ),
  customVariable("CUSTOM_SHORT_GITHUB_SHA", () =>
    process.env.GITHUB_SHA.substring(0, 7)
  ),
  customVariable("CUSTOM_PR_TITLE", () => gitHubEvents().pull_request.title),
  customVariable("CUSTOM_COMMIT_MSG", () => commitMessage()),
  customVariable(
    "CUSTOM_ACTION_LINK",
    () => `https://github.com/${repositoryName}/actions/runs/${runId}`
  ),
  customVariable("CUSTOM_GITHUB_ACTOR_AS_SLACK", () =>
    gitHubNameToSlackMention(authorName)
  ),
];
...
const escapeUnicode = (str) =>
  str.replace(
    /[^\0-~]/g,
    (ch) => "\\u" + ("000" + ch.charCodeAt().toString(16)).slice(-4)
  );
...
(async () => {
  try {
    ...
    if (!data) {
      throw new Error(
        "There is nothing to send! Please check your slack_json parameter!"
      );
    }
